...
`FORBIDDEN`（禁抛）或 `ALLOWED`（放行），并给出可复核的整数几何证据；另提供
**连续航迹裁决**：给定 2～100 个有序航点，检查相邻航点构成的航段是否穿入禁抛区，
返回 `CLEAR` 或 `BLOCKED`（含最早受限航段的首次接触证据）。
...
2. 等待 `api` 健康检查通过后，运行 `scripts/acceptance.py`，
   ...
   以及连续航迹（两端放行但中途穿区时定位首次入口、擦过顶点与沿边航行稳定阻断、
   完全绕行返回 CLEAR、起点已禁抛参数为零、零长度航段按单点规则、
...
施工船在执行连续航迹前，确认相邻航点之间不会穿入禁抛区：逐点裁决只看航点本身，
会漏掉两个放行航点之间的越界航段，本接口把有序航点连成航段逐段检查。接收现有
`region` 与 **2～100 个有序航点**（**不接收** `points`、`exclusion_margin_cm` 与
`permitted_pockets`，未声明字段同样整单 422）。
...
成功响应（`200`）：全部航段未接触禁抛区（含边界）时 `decision` 为 `CLEAR`、
`contact` 为 `null`；否则为 `BLOCKED`，`contact` 给出**最早受限航段**的首次接触：
...
- 航段 `i` 为 `waypoints[i] → waypoints[i+1]`；`contact.segment_index` 是最早
  （序号最小）接触禁抛区的航段；
- 接触点 = 航段起点 + `t` ×（终点 − 起点），`t = t_num / t_den`（`0 <= t <= 1`）；
  接触参数与接触坐标（`x_num/x_den`、`y_num/y_den`）一律为**约分有理数**，分母为正，
  全程整数计算无浮点；
- **起点已禁抛**（在禁抛区内部或边界上）时参数为零、接触点即起点：边界起点沿用
  最小边序号归因；严格内部起点的接触不涉及任何边，`edge_index` 为 `null`；
- 航段与边**共线贴边**时取重叠起点为首次接触；同一接触点命中多条边（顶点、
  重叠起点）时取**最小输入边序号**；
- **零长度航段按单点规则**处理：该点禁抛则构成参数为零的接触，在外部则不构成接触；
- 结论、航段序号、接触参数与接触坐标与区域顶点顺/逆时针方向无关（边序号随编号
  方案变化，与逐点裁决的既有约定一致）；
- 航点数量（少于 2 个或多于 100 个）、坐标或区域非法时，按统一错误信封**整单 422**，
  绝不返回部分结果。
...
| --- | --- | --- |
| `VALIDATION_ERROR` | JSON 结构错误、坐标非整数/越界、区域顶点原始条目少于 3、点数超过 500、航点数不在 2～100、安全距离非法（负数/非整数/超过坐标上限）等 | `issues`: 字段级问题列表 |
...
- **面积汇总**：矩形外环加两个口袋核对 `net_area2` 面积守恒、顺/逆时针与闭合
  ...
  大坐标二倍面积整数精确；
- **连续航迹**：两端放行但中途穿区时定位首次入口（含后续航段才进入时报最早
  受限航段）、擦过顶点与沿边航行稳定阻断（顶点命中多边取最小边序号、共线贴边
  取重叠起点）、完全绕行返回 `CLEAR`、起点已禁抛参数为零（内部起点边序号为
  `null`）、零长度航段按单点规则、真分数接触坐标约分、顺/逆时针结论与接触坐标
  不变、航点数量（2～100）/坐标/区域非法整单 422、大坐标接触参数整数精确；
- **随机性质测试**：`test_random_polygons_match_exact_reference` 生成大量随机星形简单
  ...
  （同距取最小序号）与约分后的平方距离分数；
  `test_random_tracks_match_fraction_reference` 对随机航迹对拍最早受限航段、
  首次接触参数、接触坐标与边序号，`test_random_tracks_orientation_reversal_invariant`
  验证方向反转下结论与接触坐标不变。
