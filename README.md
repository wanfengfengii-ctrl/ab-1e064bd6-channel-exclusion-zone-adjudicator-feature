...
另有连续航迹接口 `POST /adjudicate-track`，裁决有序航点构成的整条航迹是否
穿入/接触禁抛区（`CLEAR`/`BLOCKED`），阻断时定位最早受限航段与首次接触点。
...
2. 等待 `api` 健康检查通过后，运行 `scripts/acceptance.py`，
   ...
   以及连续航迹（两端放行中途穿区时定位首次入口、擦过顶点与沿边航行稳定阻断、
   完全绕行 CLEAR、分数接触参数与坐标约分、零长度航段按单点规则、
   反转区域方向不改变结论与接触坐标、航点数量/坐标/区域非法整单 422）。
...
施工船执行连续航迹前，仅靠逐点裁决会漏掉“两个放行航点之间穿入禁抛区”的航段。
本接口接收与裁决一致的 `region` 与 **2～100 个有序航点**（`waypoints`，严格整数
坐标、未声明字段整单 422），相邻航点构成一个航段（共 n−1 段，序号从 0 开始）：

- 所有航段都不接触禁抛区时返回 `{"decision": "CLEAR", "contact": null}`；
- 否则返回 `"BLOCKED"`，`contact` 给出**最早受限航段**的首次接触证据：
  - `segment_index`：最早受限航段序号（航点 i → i+1）；
  - `t_num` / `t_den`：首次接触的航段参数（约分分数），
    `P(t) = 起点 + t·(终点−起点)`，起点已禁抛时为 `0/1`；
  - `point`：接触点坐标，以约分有理数 `x_num/x_den`、`y_num/y_den` 给出
    （整数坐标时分母为 1）；
  - `edge_index`：首次接触归因的区域**输入边序号**（同点命中多边时取最小序号；
    起点严格位于区域内部、不接触任何边时为 `null`）。

规则要点：

- 几何层复用区域多边形规整、点分类、叉积与边界归因；各航段与每条边的接触
  位置全部以**约分有理数**（标准库 `fractions.Fraction`）计算并比较，无浮点；
- 航段从外部**穿越**区域（两端放行、中途穿区）时在首次入口阻断；
- **擦过顶点**（外→边界→外的相切）同样阻断，接触点即该顶点；
- **沿边航行**（共线贴边）稳定阻断，接触取航段与边**重叠区间的起点**；
- 航点本身已在区域内或边界上时，该航段在参数 0 处阻断；
- **零长度航段**（相邻航点相同）按单点规则处理：外部放行、内部/边界阻断；
- 结论与接触坐标不随区域顺/逆时针方向改变（边序号随输入方向重编号，
  但归属同一条几何边）；
- 本接口**不接收** `points`、`exclusion_margin_cm` 与 `permitted_pockets`，
  夹带这些字段与其它未声明字段一样整单 422。
...
```json
{
  ...
  "contact": {
    ...
    "t_num": 1,
    "t_den": 4,
    "point": {"x_num": 0, "x_den": 1, "y_num": 5, "y_den": 1},
    "edge_index": 3
  }
}
```

斜边斜穿的接触参数与坐标为非整数有理数时同样精确约分，例如三角形
（顶点 `(0,0),(10,0),(0,10)`）上航段 `(8,7)→(3,4)` 的首次接触为
`t = 5/8`，接触点 `(39/8, 41/8)`，归因斜边 `edge_index = 1`。
...
| --- | --- | --- |
| `VALIDATION_ERROR` | JSON 结构错误、坐标非整数/越界、区域顶点原始条目少于 3、点数超过 500、航迹航点不在 2～100 个范围内、安全距离非法（负数/非整数/超过坐标上限）等 | `issues`: 字段级问题列表 |
...
- **面积汇总**：矩形外环加两个口袋核对 `net_area2` 面积守恒、顺/逆时针与闭合
  ...
  大坐标二倍面积整数精确；
- **连续航迹**：两端放行中途穿区时返回 `BLOCKED` 并定位最早受限航段的首次入口
  （约分参数与坐标、边序号）、擦过顶点与沿边航行（共线取重叠起点）稳定阻断、
  起点已禁抛时参数为零、完全绕行 `CLEAR`、零长度航段按单点规则、2～100 航点
  数量限制、非法区域/坐标与未声明字段（含安全距离、许可口袋）整单 422；
  `test_random_tracks_match_fraction_reference` 以标准库 `fractions.Fraction`
  独立求解随机简单多边形（顺/逆时针各一遍）上的随机航迹首次接触，对拍段序号、
  参数、坐标与边序号，期望由数学参照实时计算，不固化结果；
